(function (global) {
  ...
  function analyze(rows, region) {
    ...
    const items = rows.map(row => {
      ...
      return { ...row, stockQty: onHand, available, openClient, openSupplier, avg3: avgMonthly, ats: savedAts, actualAvailable, upcomingAvailability, supplierDueQty, planningSupplierQty, netInventoryPosition, calculatedRecommendation, activeBrand, eligible, excluded, daysUntil, leadTime, leadTimeMonths, reorderRequired, reorderReason: reorderRequired ? "Formula recommendation" : "", recommended, monthsCover: avgMonthly > 0 ? available / avgMonthly : null, abc: "C", rank: 0, contribution: 0, cumulative: 0 };
    });
