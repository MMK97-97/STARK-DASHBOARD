(() => {
  ...
  const icons = {
    workspace: icon('<path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z"/>'),
    ...
    events: icon('<path d="M5 5h14v15H5zM8 3v4M16 3v4M5 10h14"/>'),
  ...
  const items = [
    ["workspace", "Workspace", `index.html?workspace=${regionCode}`, icons.workspace],
    ["dashboard", "Inventory Analysis", regional("inventory-dashboard"), icons.dashboard],
    ...
    ["freight", "Freight Estimator", "freight-estimator.html", icons.freight, regionCode === "US"],
    ["tracking", "Tracking", "shipment-tracking.html", icons.tracking, regionCode === "US"],
    ["instructions", "Instructions", regional("instructions"), icons.instructions]
  ].filter(item => item[4] !== false);
  ...
  rail.innerHTML = `
    ...
    <nav>${items.map(([key, label, href, glyph]) => `<a href="${href}" class="${key === activeKey ? "active" : ""}" ${key === activeKey ? 'aria-current="page"' : ""}>${glyph}<span>${label}</span></a>`).join("")}</nav>
    <a class="premium-rail-back" href="index.html">${icons.workspace}<span>Back to regions</span></a>`;
