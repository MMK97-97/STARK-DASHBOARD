(() => {
  ...
  const state = { data: {}, meta: {}, analysis: [], filtered: [], activeView: "overview", exceptionFilter: "", brandFilter: "" };
  ...
  function applyFilters() {
    ...
    state.filtered=state.analysis.filter(row=>(!brand||row.brand===brand)&&(!status||row.status===status)&&(!abc||row.abc===abc)&&(!xyz||row.xyz===xyz)&&(!search||[row.model,row.itemId,row.title,row.brand].some(value=>norm(value).includes(search)))&&(!state.exceptionFilter||row.exception===state.exceptionFilter));
    renderAll();
  }

  function kpi(label,value,note,kind="") { return `<article class="iar-kpi ${kind}"><small>${esc(label)}</small><strong>${esc(value)}</strong><span>${esc(note)}</span></article>`; }
  function renderKpis() {
    ...
    $("analysis-kpis").innerHTML=[kpi("Analyzed SKUs",whole.format(rows.length),`${unique(rows.map(row=>row.brand)).length} active brands`),kpi("Forecast 30D",fmt.format(forecast),"Selected by SKU backtest","good"),kpi("Stockout risk",whole.format(risks),"Urgent, reorder or program risk","risk"),kpi("Recommended units",whole.format(recommended),"Lead-time demand + safety stock","warn"),kpi("On-hand units",whole.format(stock),"Eligible active-brand inventory"),kpi("Excess units",whole.format(excess),"Above four months plus safety stock","warn"),kpi("Forecast accuracy",accuracy==null?"—":pct(accuracy),accuracy==null?"Insufficient backtest history":"1 − portfolio WAPE"),kpi("Inventory value",state.data.priceCost?.length?money.format(inventoryValue):"—",state.data.priceCost?.length?"At uploaded unit Cost":"Upload Price & Cost list")].join("");
  }
  ...
  function donut(entries,totalLabel) { const total=sum(entries.map(entry=>entry.value))||1;let cursor=0;const stops=[];entries.forEach((entry,index)=>{const start=cursor/total*100;cursor+=entry.value;stops.push(`${colors[index%colors.length]} ${start}% ${cursor/total*100}%`);});return `<div class="donut-wrap"><div class="donut" style="background:conic-gradient(${stops.join(",")})"><span class="donut-center"><strong>${whole.format(total)}</strong><span>${esc(totalLabel)}</span></span></div><div class="legend-list">${entries.map((entry,index)=>`<button type="button" class="legend-item" data-exception="${esc(entry.name)}"><i style="background:${colors[index%colors.length]}"></i><span>${esc(entry.name)}</span><strong>${whole.format(entry.value)}</strong></button>`).join("")}</div></div>`; }
  function lineChart(actual,forecast,labels) {
    ...
    return `<svg class="trend-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Demand and forecast trend">${[0,.25,.5,.75,1].map(r=>`<line class="grid" x1="${pad}" y1="${y(max*r)}" x2="${width-pad}" y2="${y(max*r)}"/><text x="0" y="${y(max*r)+3}">${fmt.format(max*r)}</text>`).join("")}<path class="actual" d="${path(actual)}"/>${forecast.length?`<path class="forecast" d="${path(forecast)}"/>`:""}${actual.map((value,index)=>Number.isFinite(value)?`<circle cx="${pad+index*step}" cy="${y(value)}" r="3"/>`:"").join("")}${labels.map((label,index)=>`<text x="${pad+index*step}" y="${height-5}" text-anchor="middle">${esc(label)}</text>`).join("")}</svg>`;
  }
  function renderOverview() {
    ...
    $("trend-chart").innerHTML=lineChart([...actual,NaN],[...Array(Math.max(0,actual.length-1)).fill(NaN),actual.at(-1)||0,forecast],[...labels,"Next"]);
    const exceptions=grouped(rows,"exception",[]).map(row=>({name:row.name,value:row.count})).sort((a,b)=>b.value-a.value);$("exception-chart").innerHTML=donut(exceptions,"SKUs");
    const brands=grouped(rows,"brand",["forecast30"]).sort((a,b)=>b.forecast30-a.forecast30);$("brand-demand-chart").innerHTML=barChart(brands,"forecast30","name",fmt.format,9,true);
    const matrix=new Map();rows.forEach(row=>{const key=`${row.abc}${row.xyz}`;matrix.set(key,(matrix.get(key)||0)+1);});const matrixRows=[...matrix].map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value);$("abc-xyz-chart").innerHTML=donut(matrixRows,"SKUs");
  ...
  async function handleUpload(event) {
    ...
    try{const parsed=await normalizeUpload(file,type),stored={rows:parsed.rows,meta:{...parsed.meta,name:file.name,rows:parsed.meta.rawRows||parsed.rows.length,uploadedAt:new Date().toISOString()}};await saveStored(type,stored);state.data[type]=stored.rows;state.meta[type]=stored.meta;if(type==="itemStock")state.meta.months=stored.meta.months;localStorage.removeItem(`stark-analysis-seed-disabled-${regionCode}`);populateFilters();computeAnalysis();renderSourceStatus();toast(`${file.name} validated and applied.`);}catch(error){state.meta[type]={name:file.name,rows:0,invalid:error.message};renderSourceStatus();renderQuality();toast(error.message,true);}finally{input.value="";}
  }
  ...
  async function initData() {
    ...
    const disabled=localStorage.getItem(`stark-analysis-seed-disabled-${regionCode}`)==="true",seed=!disabled?window.STARK_INVENTORY_ANALYSIS_SEED?.[regionCode]:null;
    if(seed){state.meta={asOf:seed.asOf,months:seed.months,...seed.sources};sourceTypes.forEach(type=>{if(seed[type])state.data[type]=seed[type];});state.meta.itemStock={...(state.meta.itemStock||{}),months:seed.months};}
  ...
  function bind() {
    ...
    ["filter-brand","filter-status","filter-abc","filter-xyz"].forEach(id=>$(id).addEventListener("change",()=>{state.exceptionFilter="";state.brandFilter="";applyFilters();}));$("filter-search").addEventListener("input",()=>{state.exceptionFilter="";applyFilters();});
    $("reset-analysis-filters").addEventListener("click",()=>{["filter-brand","filter-status","filter-abc","filter-xyz","filter-search"].forEach(id=>$(id).value="");state.exceptionFilter="";state.brandFilter="";applyFilters();});
    ...
    document.addEventListener("click",event=>{const ex=event.target.closest("[data-exception]");if(ex){state.exceptionFilter=state.exceptionFilter===ex.dataset.exception?"":ex.dataset.exception;applyFilters();}const bar=event.target.closest("[data-bar-value]");if(bar){state.brandFilter=state.brandFilter===bar.dataset.barValue?"":bar.dataset.barValue;$("filter-brand").value=state.brandFilter;applyFilters();}const exportButton=event.target.closest("[data-export-table]");if(exportButton)exportWorkbook();});
    $("export-analysis").addEventListener("click",()=>exportWorkbook());
    $("clear-analysis").addEventListener("click",async()=>{if(!confirm(`Clear all Inventory Analysis Report uploads for ${regionName}? Existing Inventory Dashboard data will not be changed.`))return;await Promise.all(sourceTypes.map(deleteStored));localStorage.setItem(`stark-analysis-seed-disabled-${regionCode}`,"true");state.data={};state.meta={};state.analysis=[];state.filtered=[];populateFilters();renderSourceStatus();renderAll();toast("Regional analysis sources cleared.");});
