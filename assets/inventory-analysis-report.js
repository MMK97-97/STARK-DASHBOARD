(() => {
  ...
  function renderOverview() {
    ...
    const coverage=[{name:"Critical <0.5",test:r=>r.mos!=null&&r.mos<.5},{name:"Low 0.5–1",test:r=>r.mos>=.5&&r.mos<1},{name:"Healthy 1–2",test:r=>r.mos>=1&&r.mos<2},{name:"Watch 2–4",test:r=>r.mos>=2&&r.mos<=4},{name:"Excess >4",test:r=>r.mos>4},{name:"No demand",test:r=>r.mos==null}].map(band=>({name:band.name,value:rows.filter(band.test).length}));$("abc-xyz-chart").innerHTML=donut(coverage,"SKUs",false);
