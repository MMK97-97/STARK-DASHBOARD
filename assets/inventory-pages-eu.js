(function () {
  ...
  function filteredReorders() { const brand = el("reorder-brand").value, search = el("reorder-search").value.trim().toLowerCase(); return items.filter(item => item.reorderRequired).filter(item => (!brand || item.brand === brand) && (!search || `${item.model} ${item.product}`.toLowerCase().includes(search))).sort((a, b) => String(a.brand).localeCompare(String(b.brand), undefined, { sensitivity: "base" }) || b.recommended - a.recommended || String(a.model).localeCompare(String(b.model), undefined, { numeric: true, sensitivity: "base" })); }
